# DescomplicAi-Simplificando-GIT
...
**`git-archimport`** - Importa um repositório GNU Arch no Git.
**`git-cvsexportcommit`** - Exporta um único commit para uma averiguação do CVS.
**`git-cvsimport`** - Recupera dados de outros sistemas de controle de versão.
**`git-cvsserver`** - Emula um servidor CVS para o Git.
**`git-imap-send`** - Envie patches por email para um servidor IMAP.
**`git-p4`** - Importa e envia para repositórios Perforce.
**`git-quiltimport`** - Aplica um conjunto de patches no ramo atual.
**`git-request-pull`** - Gera um resumo com as modificações pendentes.
**`git-send-email`** - Envia patches como e-mails.
**`git-svn`** - Integração entre repositórios do Subversion e Git.

**REDEFINA, RESTAURE REVERTA:**

Existem três comandos semelhantes com nomes diferentes:

**`git-revert`** - Cria um novo commit que desfaz as alterações de outros commits.
**`git-restore`** - Restaura arquivos na sua árvore de trabalho ou no índice sem afetar seu histórico de commits.
**`git-reset`** - Move o seu branch, adicionando ou removendo commits, o que modifica o histórico de commits. Pode ser usado para restaurar o índice.

**COMANDOS DE NÍVEL BAIXO (Encanamento Plumbing):**
...
**`git-apply`** - Aplica um patch nos arquivos ou no índice.
**`git-checkout-index`** - Copia os arquivos do índice para a árvore de trabalho.
**`git-commit-graph`** - Escreve e verifica arquivos commit-graph do Git.
**`git-commit-tree`** - Cria um novo objeto de commit.
**`git-hash-object`** - Calcula o ID de um objeto a partir de um arquivo.
**`git-index-pack`** - Constrói um pacote de índice a partir de um arquivo de pacote existente.
**`git-merge-file`** - Realiza uma mesclagem de três vias em um arquivo.
**`git-merge-index`** - Realiza mesclagens em arquivos que precisam ser mesclados.
**`git-mktag`** - Cria um objeto de tag.
**`git-mktree`** - Cria uma árvore-objeto a partir de um arquivo de formato ls-tree.
**`git-multi-pack-index`** - Escreve e verifica vários índices de pacotes.
**`git-pack-objects`** - Cria um histórico empacotado de objetos.
**`git-prune-packed`** - Remove objetos extras de arquivos empacotados.
**`git-read-tree`** - Lê informações da árvore no índice.
**`git-symbolic-ref`** - Lê, modifica e exclui referências simbólicas.
**`git-unpack-objects`** - Desempacota objetos de um arquivo empacotado.
**`git-update-index`** - Registra o conteúdo de um arquivo na árvore de trabalho no índice.
**`git-update-ref`** - Atualiza o nome de um objeto armazenado de forma segura.
**`git-write-tree`** - Cria um objeto de árvore com base no índice atual.
...
**`git-cat-file`** - Fornece informações sobre objetos no repositório.
**`git-cherry`** - Procura commits que ainda não foram aplicados ao "upstream".
**`git-diff-files`** - Compara arquivos na árvore de trabalho e no índice.
**`git-diff-index`** - Compara uma árvore com o diretório de trabalho ou o índice.
**`git-diff-tree`** - Compara o conteúdo e o modo de bolhas através de dois objetos de árvore.
**`git-for-each-ref`** - Fornece informações sobre cada "ref".
**`git-get-tar-commit-id`** - Extrai o ID de commit de um arquivo criado com o git-archive.
**`git-ls-files`** - Exibe informações sobre arquivos no índice e na árvore de trabalho.
**`git-ls-remote`** - Lista referências em um repositório remoto.
**`git-ls-tree`** - Lista o conteúdo de uma árvore de objetos.
**`git-merge-base`** - Localiza os melhores ancestrais para fazer uma mesclagem.
**`git-name-rev`** - Localiza nomes simbólicos para "revs" informados.
**`git-pack-redundant`** - Localiza arquivos "pack" redundantes.
**`git-rev-list`** - Lista objetos de commit em ordem cronológica reversa.
**`git-rev-parse`** - Escolhe e modela parâmetros.
**`git-show-index`** - Exibe o índice de um arquivo empacotado.
**`git-show-ref`** - Lista referências em um repositório local.
**`git-unpack-file`** - Cria um arquivo temporário com conteúdos de objetos.
**`git-var`** - Exibe variáveis locais para o Git.
**`git-verify-pack`** - Valida arquivos empacotados do Git.
...
**`git-daemon`** - Um servidor simples para hospedar repositórios Git.
**`git-fetch-pack`** - Recebe objetos ausentes de outro repositório.
**`git-http-backend`** - Implementa o Git como servidor através do protocolo HTTP.
**`git-send-pack`** - Empurra objetos para outro repositório usando o protocolo Git.
**`git-update-server-info`** - Atualiza informações auxiliares para auxiliar servidores.

**COMANDOS AUXILIARES DE SICRONIZAÇÃO:**
...
**`git-http-fetch`** - Faz o download de um repositório Git remoto via HTTP.
**`git-http-push`** - Empurra objetos via HTTP/DAV para outro repositório.
**`git-parse-remote`** - Ajuda na análise dos parâmetros de acesso a repositórios remotos.
**`git-receive-pack`** - Recebe objetos que são empurrados para o repositório.
**`git-shell`** - Fornece um shell de login restrito para acessar repositórios Git via SSH.
**`git-upload-archive`** - Envia um arquivo de volta ao git-archive.
**`git-upload-pack`** - Envia objetos compactados para o git-fetch-pack.
...
**`git-check-attr`** - Exibe informações dos atributos do Git.
**`git-check-ignore`** - Verifica regras de .gitignore e exclui arquivos correspondentes.
**`git-check-mailmap`** - Exibe informações canônicas de contatos em mensagens.
**`git-check-ref-format`** - Garante que um nome de referência esteja bem formatado.
**`git-column`** - Exibe dados em formato de coluna.
**`git-credential`** - Obtém e armazena credenciais de usuário.
**`git-credential-cache`** - Auxilia no armazenamento temporário de senhas na memória.
**`git-credential-store`** - Auxilia no armazenamento de credenciais no disco.
**`git-fmt-merge-msg`** - Gera mensagens de mesclagem de commits.
**`git-interpret-trailers`** - Adiciona ou analisa informações estruturadas em mensagens de commit.
**`git-mailinfo`** - Extrai remetentes e correções de mensagens de e-mail.
**`git-mailsplit`** - Divide arquivos mbox do UNIX em mensagens individuais.
**`git-merge-one-file`** - Assistente para mesclagem de arquivos individuais.
**`git-patch-id`** - Calcula um ID único para um patch.
**`git-sh-i18n`** - Configuração internacionalização do Git para scripts shell.
**`git-sh-setup`** - Configuração comum para scripts shell do Git.
**`git-stripspace`** - Remove espaços em branco desnecessários.
...
**CONFIGURAÇAO DE COMMITS NO GIT:**
